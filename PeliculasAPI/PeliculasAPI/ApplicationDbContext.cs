...
namespace PeliculasAPI
{
    public class ApplicationDbContext: DbContext
    {
        ...
        {
            // Crear llaves primarias compuestas 

            modelBuilder.Entity<PeliculasActores>()
                .HasKey(x => new { x.ActorId, x.PeliculaId }); // Primary Key (ActorID, PeliculaId)
            modelBuilder.Entity<PeliculasGeneros>()
                .HasKey(x => new { x.GeneroId, x.PeliculaId}); // Primary Key (GeneroId, PeliculaId)
            modelBuilder.Entity<PeliculasSalasDeCine>()
                .HasKey(x => new { x.PeliculaId, x.SalaDeCineId }); // Primary key (PeliculaId, SalaDeCineId)
        ...
        private void SeedData(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<PeliculasActores>().HasData(
                ...
                {
                    new PeliculasActores(){PeliculaId = endgame.Id, ActorId = robertDowney.Id, Personaje = "Tony Stark", Order = 1},
                    new PeliculasActores(){PeliculaId = endgame.Id, ActorId = chrisEvans.Id, Personaje = "Steve Rogers", Order = 2},
                    new PeliculasActores(){PeliculaId = iw.Id, ActorId = robertDowney.Id, Personaje = "Tony Stark", Order = 1},
                    new PeliculasActores(){PeliculaId = iw.Id, ActorId = chrisEvans.Id, Personaje = "Steve Rogers", Order = 2},
                    new PeliculasActores(){PeliculaId = sonic.Id, ActorId = jimCarrey.Id, Personaje = "Dr. Ivo Robotnik", Order = 1}
                });
        }
